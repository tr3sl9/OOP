...
‣ получить всех «отстающих» студентов, у которых три и более двойки, используя класс-итератор

### Многопоточность:
‣ **получить всех отстающих студентов (многопоточно)** - каждая группа обрабатывается в отдельном потоке \
‣ **вычислить средний балл по всем группам (многопоточно)** - параллельная обработка групп \
‣ См. подробную документацию в [MULTITHREADING.md](MULTITHREADING.md)
...
## запуск бенчмарка производительности (многопоточность)
```
cd build/src/client
./benchmark
```

После запуска бенчмарка будет создан файл `benchmark_results.csv` с результатами измерений.

Для построения графиков производительности:
```
